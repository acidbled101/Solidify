...
Run once after cloning TRELLIS.2:
    python third_party/patches/mps_compat.py
"""
...
# This file lives at third_party/patches/, so the repo root is two levels up
# (patches -> third_party -> root) and backends/ is its sibling inside
# third_party. Only these path constants changed when the upstream port was
# quarantined; nothing about what the patches DO was touched.
_THIRD_PARTY = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_REPO_ROOT = os.path.dirname(_THIRD_PARTY)

# Run directly (`python3 third_party/patches/mps_compat.py`) sys.path[0] is this
# file's own directory, so `from backends.stubs import ...` below needs help.
sys.path.insert(0, _THIRD_PARTY)

TRELLIS_ROOT = os.path.join(_REPO_ROOT, "TRELLIS.2")
BACKENDS_DIR = os.path.join(_THIRD_PARTY, "backends")
