...
from training.train_run import TrainRun, Control
from trellis_core import lora as lora_mod
from .flow_matching import (checkpointed_blocks, noise_latent, velocity_target,
                            sample_timesteps, FlowMatchConfig)
...
def flow_matching_loss_batch(model, samples, cfg: FlowMatchConfig, p_uncond: float = 0.0):
...
def flow_matching_loss(model, coords, feats, cond, cfg: FlowMatchConfig,
...
@torch.no_grad()
def evaluate(model, pipeline, ds: SlatDataset, cfg: FlowMatchConfig, *,
...
def main(argv=None) -> int:
    ...
    cfg = FlowMatchConfig(sigma_min=pipeline.shape_slat_sampler.sigma_min,
