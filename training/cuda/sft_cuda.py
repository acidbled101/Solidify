...
WHAT THIS IS
------------
A port of training/sft_train.py with three changes and nothing else moved:
...
class SlatDataset(torch.utils.data.Dataset):
    """(latent, conditioning) pairs written by training/dataset_build.py.
        ...
        for batch in loader:
            ...
            if step % args.log_every == 0 or args.smoke:
                ...
                run.write_json("status.json", {
                    # run_id/pid/heartbeat are what experiments/dpo_inference_steering/inspector/train_server.py reads
