...
| Component | File | Size |
|---|---|---|
| Geometric judge (S = αR_Detail − βL_OH − γL_Th − δL_Topo) | `trellis_core/geometric_judge.py` | 8 tests |
| Fork / steer / re-rank / resume sampler | `trellis_core/dpo_branch.py` | 1785 lines, 23 tests |
| Flow-matching DPO theory + 2-D validation | `trellis_core/flow_dpo.py` | 15 tests |
| Differentiable printability objectives | `trellis_core/printability_proxy.py` | 10 tests |
| Live inspector dashboard + run harness | `devlab/` | 4 tests |
| Pooled-run analysis | `devlab/metrics.py` | 378 lines |

60 tests total. 22 recorded runs in `devlab/traces/`.
...
2. **A 2-D toy** (`devlab/alignment_experiment.py`, ~104 s, n=3000/model) with a
   closed-form geometry family, where the *alignment mechanism* can be isolated
   from the 4B-parameter pipeline.
3. **Pooled real runs** (`devlab/metrics.py`) flattening every trace on disk
...
Pooled across every real fork on disk (`devlab/traces/metrics.json`, n=17 scored
...
- **`devlab/`** — the run harness, trace format, and pooled-metrics analysis
