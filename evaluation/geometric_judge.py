...
non-ML print-prep pipeline in trellis_core/printprep/printable.py. Where a metric
...
def overhang_penalty(mesh: trimesh.Trimesh, theta_crit_deg: float = 45.0) -> float:
    ...
    trellis_core/printprep/printable.py's diagnostics() overhang convention
...
class JudgeDetails:
    ...
    visualization (see experiments/dpo_inference_steering/inspector/) can show histograms and per-element
...
def score_mesh_detailed(
    ...
    normally discards -- for visualization/debugging (see experiments/dpo_inference_steering/inspector/).
