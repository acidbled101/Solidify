...
(third_party/backends/mesh_extract.py does `.item()` and Python dict lookups on CPU). Those
...
class DPOBranchReport:
    ...
    # already constructing DPOBranchReport by keyword. See experiments/dpo_inference_steering/inspector/ for the
...
class DPOMultiBranchReport:
    ...
    outside experiments/dpo_inference_steering/inspector/ reads dpo_generation.py's dpo_report's fields directly
...
def sparse_conv_backend(backend: str = "none"):
    ...
    while third_party/backends/conv_none.py is plain gather/scatter PyTorch and
    ...
    try:
        try:
            ...
            # A "No module named ...conv_none" here means third_party/patches/mps_compat.py
            # has not been run against this TRELLIS.2 checkout -- that script is
            # what copies third_party/backends/conv_none.py into the trellis2 tree. Still
...
def voxel_neighbor_pairs(coords: torch.Tensor, grid_stride: int = 2048) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    deliberately NOT the Python dict loop that third_party/backends/conv_none.py uses to
...
def steer_delta(
    ...
    *,
    ...
    experiments/dpo_inference_steering/inspector/ to stream per-gradient-step events live rather than waiting for
...
def differentiable_continuation(
    ...
    **inference_kwargs,
    ...
    experiments/dpo_inference_steering/inspector/ trace where the reference and delta branches actually diverge
...
def _decode_candidate(pipeline, slat_normalized, normalization, resolution) -> Optional[trimesh.Trimesh]:
    ...
    non-differentiable anyway (third_party/backends/mesh_extract.py uses .item() and Python
...
def _rank_detailed(mesh_a, mesh_b, weights, rng):
    ...
    visualization (see experiments/dpo_inference_steering/inspector/). _rank() is a thin wrapper that drops these
...
def sample_shape_slat_with_dpo_branch(
    ...
    Args:
        ...
        on_event: optional callback(event_type: str, payload: dict), called
            ...
            and the resume decision) -- see experiments/dpo_inference_steering/inspector/ for the consumer this
