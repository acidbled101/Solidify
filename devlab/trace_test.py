...
from devlab.trace import TraceReader, TraceWriter, _histogram
...
def test_mesh_export_includes_stl_sibling():
    ...
    devlab/static/app.js's download links)."""
    d = _tmpdir()
    try:
        w = TraceWriter(d)
        box = trimesh.creation.box(extents=[1, 1, 1])
